	...
	void *params, chunk_t data, chunk_t signature)
{
	...
	if (wc_dilithium_verify_msg(signature.ptr, signature.len, data.ptr,	data.len,
								&result, &this->key) != 0)
	{
		DBG1(DBG_LIB, "signature verification with scheme %N failed",
					   signature_scheme_names, scheme);
		return FALSE;
	}
...
public_key_t *wolfssl_ml_dsa_public_key_load(key_type_t type, va_list args)
{
	...
	if ((blob.len == 0 && asn1.len == 0) ||
		!wolfssl_ml_dsa_enabled(type, &level))
	{
		return NULL;
	}
	...
	{
		if (wc_Dilithium_PublicKeyDecode(asn1.ptr, &idx, &this->key, asn1.len) != 0)
		{
